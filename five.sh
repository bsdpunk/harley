...
do
   ...
   #Get Body
    ...
    if [[  -z $NAME ]]
    then
    NAME=$( grep $n b.txt  -B10 | grep '[A-Z]\.' | head -n1 | sed 's/^[A-Z]\. //' | sed 's/[A-Z][a-z].*//g')
        fi
    #Get Handle
    HANDLE=$( echo $NAME | tr '[A-Z]' '[a-z]' | sed 's/ /-/g' | sed "s/$/-${n}/"| sed 's/[A-Z][a-z]//' )
 
        #Get Price
    PRICE=$( grep "$n" b.txt | ggrep -P -m1 -o '\$\d+\.\d+' | sed  's/\$//' )
    if [[  -z $PRICE ]]
    then
        if [[ "$n" -eq "$TEMP" ]]
        then
            PRICE=$(grep "$n" b.txt -A10 | ggrep -P -o '\$\d+\.\d+' | sed 's/\$//g' | tr ' ' '\n' |tail -n1)
        else

            PRICE=$(grep "$n" b.txt -A6 | ggrep -P -o '\$\d+\.\d+' | sed 's/\$//g' |tr ' ' '\n' | head -n1)
        fi
    fi
